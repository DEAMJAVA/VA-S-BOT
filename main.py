...
if not os.path.isfile('BotConfig.json'):
  ...
  with open('BotConfig.json', 'w') as f:
    f.write(
      json.dumps(config, indent=4, ensure_ascii=False, separators=(',',
                                                                   ': ')) +
      '\n')
  messagebox.showinfo(title="Va's BOT",
  ...
  try:
    ...
    messagebox.showinfo(title="Va's BOT",
                        message="Please Regenerate Config file",
                        icon=messagebox.INFO)
  ...
  if str(config['plugins']) == "True":
    ...
    plugin_files = [
      f for f in os.listdir(plugins_folder)
      if os.path.isfile(os.path.join(plugins_folder, f))
    ]
  ...
  def has_required_perm():
    ...
    async def predicate(ctx):
      try:
        ...
        if str(owner_id) != str(
            ctx.author.id) and mod_role not in ctx.author.roles and str(
              creator_id) != str(ctx.author.id):
          await ctx.send(
            "You don't have the required permision to execute this command!")
          return False
  ...
  def has_owner_perm():
    ...
    async def predicate(ctx):
      ...
      if str(owner_id) != str(
          ctx.author.id) and owner_role not in ctx.author.roles and str(
            creator_id) != str(ctx.author.id):
        await ctx.send(
  ...
  async def status(ctx, arg: str):
    try:
      if arg == 'online':
        ...
        await ctx.send(
          "Invalid status provided. Please choose from 'online', 'invisible', 'idle', or 'dnd'."
        )
    except Exception as e:
      await ctx.send(f'An error occured: {e}')
  ...
  async def random(ctx, mi=None, ma=None):
    try:
      ...
      if mi == None or ma == None or int(mi) >= int(ma) or not isinstance(
        ...
        await ctx.send(
          f'Incorrect usage. Please use `{prefix}random [minimum] [maximum]` with integer values.'
        )
        return
  ...
  async def removerole(ctx, member: discord.Member, role: discord.Role):
    try:
      ...
      await ctx.send(
        f"{member.display_name} has had the {role.name} role removed.")
    except Exception as e:
      await ctx.send(f"An error occurred: {e}")
  ...
  async def createrole(ctx, role_name):
    try:
      ...
      permissions = discord.Permissions(
        send_messages=True,
        read_messages=True)  # set desired permissions for the role
      await guild.create_role(name=role_name,
                              permissions=permissions)  # create the role
  ...
  async def changerolecolor(ctx, role: discord.Role, hex_color: str):
    try:
      color = discord.Colour(
        int(hex_color.strip("#"),
            16))  # convert hex color string to a discord.Color object
  ...
  async def register(ctx):
    if str(ctx.author.id) not in balances:
      ...
      await ctx.send(
        f"{ctx.author.mention}, you have been registered with a starting balance of 100 coins."
      )
    else:
  ...
  async def give(ctx, recipient: discord.Member, amount: int):
    if str(ctx.author.id) not in balances:
      await ctx.send(
        f"{ctx.author.mention}, you must register before giving coins.")
    elif str(recipient.id) not in balances:
      await ctx.send(
        f"{ctx.author.mention}, the recipient must register before receiving coins."
      )
    elif balances[str(ctx.author.id)] < amount:
      await ctx.send(
        f"{ctx.author.mention}, you don't have enough coins to give {amount} coins."
      )
    else:
      ...
      await ctx.send(
        f"{ctx.author.mention} gave {recipient.mention} {amount} coins.")
  ...
  async def cf(ctx, bet: int):
    ...
    if user_id not in balances:
      ...
      with open("balances.json", "w") as f:
        json.dump(balances, f)
      await ctx.send(
        f"{ctx.author.mention}, you {result} Your balance is now {balances[user_id]} coins."
      )
  ...
  async def loan(ctx):
    ...
    if user_id in loan_data:
      ...
      if time_since_loan < 86400:
        ...
        await ctx.send(
          f"Sorry {ctx.author.mention}, you can take another loan in {time_left_str}."
        )
        return
      else:
  ...
  async def returnloan(ctx):
    ...
    if balances[user_id] < amount:
      await ctx.send(
        f"{ctx.author.mention}, you don't have enough coins to return {interest} coins."
      )
      return
    else:
  ...
  async def loadloans(ctx):
    ...
    await ctx.send(
      f"Loans have been loaded from file and applied to user balances.")
